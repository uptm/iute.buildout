...
class IUTETools(BrowserView):
    ...
    @memoize
    def iute_login_url(self):
        portal_root = self.portal_state.portal()
        if getattr(portal_root, 'iute_login_without_ssl', False):
            return self.context.absolute_url() + "/login_form"
        else:
            return self.context.absolute_url().replace('http://','https://') + "/login_form"

    @memoize_contextless
    def iute_login_without_ssl(self):
        portal_root = self.portal_state.portal()
        
        return getattr(portal_root, 'iute_login_without_ssl', False)
            
    @memoize_contextless
    def iute_home_url(self):
        portal_root = self.portal_state.portal()
        if portal_root.get('use_iute_home_url', False):
            return portal_root.get('iute_home_url', None)
        else:
