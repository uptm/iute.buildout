...
def _error_html(self, title, body):
    try:
        ...
        to_ = pp.site_properties.getProperty('error_recipients', ['flamelcanto@gmail.com', ])
        req = site.REQUEST
        ...
        if self.status != 500 and (monkey.last_sent_404 + one_day) <= now:
            ...
            if hasattr(mailhost, 'secureSend'):
                mailhost.secureSend(body, to_, 'flamelcanto@gmail.com', 'Plone IUTE WebSite ERROR!')
            else:
                mailhost.send(body, to_, 'flamelcanto@gmail.com', 'Plone IUTE WebSite ERROR!')
